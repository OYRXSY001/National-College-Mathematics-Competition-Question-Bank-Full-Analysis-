...
> **🚀 第二版（v2.0）重点更新**：新增**免安装 Windows 桌面版**——`dist/数学竞赛题库.exe` 双击即用、无需安装 Python，自动弹出浏览器进入站点；同时上线**练习模式、模拟考试、错题本间隔复习、智能组卷**等完整学习闭环。

## 项目简介

本项目是面向全国大学生数学竞赛（非数学类 A/B）的真题题库系统，提供 **Windows 桌面版（exe）**、**Web 网站**两种使用方式：

- **Web 端（Django）**：在线浏览第 1–17 届初赛/决赛真题试卷与解析，公式使用 KaTeX 渲染，支持下载试卷 PDF，后台支持按届导入与管理题库数据。
- **桌面版（exe，第二版重点）**：`数学竞赛题库.exe` 双击即用，免安装 Python，数据固定存放在用户数据目录，详见 [方式零：双击 exe](#方式零双击-exe收件人零依赖推荐)。
...
- **小程序端**：微信小程序版题库，内置题目数据与公式渲染组件，支持按试卷、知识点分类浏览和关键词搜索（源码自 v2.0 起不随本仓库分发）。
...
├── dist/数学竞赛题库.exe   # Windows 桌面版（v2.0 起随仓库发布，双击即用）
